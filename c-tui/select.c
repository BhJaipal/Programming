		...
		if (style.background.is_24_256_reset_bit == style.foreground.is_24_256_reset_bit) {
			if (style.foreground.is_24_256_reset_bit == 1) {
				...
				sprintf(esc, "\x1b[%d;%dm", style.foreground.col24.colorFlag,
						style.background.col24.colorFlag);
			} else {
			...
			if (style.foreground.is_24_256_reset_bit == 0) {
				sprintf(esc, "\x1b[38;5;%dm\x1b[%dm", style.foreground.col256, style.background.col24.colorFlag);
			} else {
				sprintf(esc, "\x1b[%dm\x1b[48;5;%dm", style.foreground.col256, style.background.col24.colorFlag);
			}
		}
		return esc;
	}
